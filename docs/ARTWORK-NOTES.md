...
**Where they render.** The work page grows a NOTES section below Details and Traits: the artist first, then whoever holds the work, then the collectors writing from outside it, newest first, folded to a count above three. Provenance notes sit in their own fold underneath it rather than inside the existing Provenance block — they are notes, and they read better beside the other notes than beside the contract address. A collector page carries the line, counting only what a stranger may read so the number never hints at a private note. `/notes` is the stream, and `/notes?by=0x…` is one person's, which is what the collector line links to.
