...
**Animated SVG** ... PixelArcade's works are SVGs that animate themselves with CSS keyframes, no script. They are read from the project's own public repo through jsDelivr rather than from IPFS, which was returning 504 from every gateway. The bytes were checked against the five copies already in this repo and are identical, the host sends `image/svg+xml` with CORS, and the chain URL stays on the record as `digital.image`. The work page now loads the SVG in an `object` so its own stylesheet runs, with a proxied still as the child the browser falls back to. The animated version is the work; the still is the fallback. Grids keep the proxied still, since 48 live SVGs is a different proposition to one.
...
- **Images have a source order.** R2 when it holds the master, then `digital.image_source`, a reliable mirror of the same bytes, then whatever the chain points at. PixelArcade is the first to use the middle one.
